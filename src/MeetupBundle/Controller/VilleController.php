...
class VilleController extends Controller
{
    public function showAction($ville)
    {
        ...
        // Date dernière extraction

        $request_extr = $em->getRepository('MeetupBundle:GroupesPHP')->createQueryBuilder('ex');
        $request_extr
            ->select('ex.date')            
            ->orderBy('ex.date', 'DESC')
            ->setMaxResults(1);

        $query_extr = $request_extr->getQuery();
        $result_extr = $query_extr->getResult();

        //date_default_timezone_set('Europe/Paris');
        
        

        //var_dump($result_extr);exit;
